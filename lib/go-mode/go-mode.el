  ...
  (condition-case nil
      (let ((file (car (godef--call point))))
        (cond
         ...
         (t
          (push-mark)
          (setq gocscope-marker (point-marker))
          (if gocscope-marker
              (progn ;; The search was successful.  Save the marker so it
                ;; can be returned to by pycscope-pop-mark.
                (if (not (equal gocscope-marker gocscope-last-marker))
                    (ring-insert gocscope-marker-ring gocscope-marker))
                (if (not (equal gocscope-marker gocscope-last-marker))
                    (setq gocscope-last-marker (point-marker)))
                ;; Unset gocscope-marker so that moving between matches
                ;; (gocscope-next-symbol, etc.) does not fill
                ;; gocscope-marker-ring.
                (setq gocscope-marker nil)))
          (godef--find-file-line-column file))))
    (file-error (message "Could not run godef binary"))))


;;-------------go-pop-mark func add by mosp, 2014/08/31 17:33:33  start---------------------------
(defvar gocscope-marker-ring-length 16
  "Length of the gocscope marker ring.")
(defvar gocscope-marker-ring (make-ring pycscope-marker-ring-length)
  "Ring of markers which are locations from which pycscope was invoked.")
(defvar gocscope-output-buffer-name "*gocscope*"
  "The name of the gocscope output buffer.")
(defvar gocscope-marker nil
  "The location from which cscope was invoked.")
(defvar gocscope-last-marker nil
  "The location from which cscope was invoked.")
(defvar gocscope-marker-window nil
  "The window which should contain gocscope-marker.  This is the window from
which gocscope-marker is set when searches are launched from the *gocscope*
buffer.")


(defun gocscope-pop-mark ()
  ...
  (if (ring-empty-p gocscope-marker-ring)
      (error "There are no marked buffers in the gocscope-marker-ring yet"))
  (let* ( (marker (ring-remove gocscope-marker-ring 0))
          (old-buffer (current-buffer))
          (marker-buffer (marker-buffer marker))
          marker-window
          (marker-point (marker-position marker))
          (gocscope-buffer (get-buffer gocscope-output-buffer-name)) )

    ;; After the following both gocscope-marker-ring and gocscope-marker will be
    ;; in the state they were immediately after the last search.  This way if
    ;; the user now makes a selection in the previously generated *gocscope*
    ;; buffer things will behave the same way as if that selection had been
    ;; made immediately after the last search.
    (setq gocscope-marker marker)

    (if marker-buffer
        (if (eq old-buffer gocscope-buffer)
            (progn ;; In the *gocscope* buffer.
