...
(defcustom zjl-hl-elisp-mode-enable-flag t
  ...
  :type 'boolean :group 'zjl-hl)
(defcustom zjl-hl-go-mode-enable-flag t
  "Enable go mode highlight when zjl-hl-enable-global-all is called"
  :type 'boolean :group 'zjl-hl)
...
(setq zjl-hl-go-mode-types-regexp
      (concat
       "\\_<[_a-zA-Z][_a-zA-Z0-9]*_t\\_>" "\\|"       
       (concat "\\_<" (regexp-opt '("bool" "int8" "byte" "int16" "uint" "uintptr" "float32" "float64" "complex64" "complex128" "string" "rune" "error" "pointer" "array" "slice" "map" "map" "chan" "struct" "interface") 'words) "\\_>")))
...
(setq zjl-hl-go-mode-keywords (list
      '("->"
        0  zjl-hl-member-point-face keep) ;;should put before the c-operators
      (cons zjl-hl-operators-regexp (cons 0  '(zjl-hl-operators-face keep)))       
      (cons zjl-hl-brackets-regexp 'zjl-hl-font-lock-bracket-face)
      (cons zjl-hl-go-mode-types-regexp 'font-lock-type-face)
      (cons zjl-hl-warning-words-regexp 'font-lock-warning-face)
      '("\\(\\_<\\(?:\\(?:0x[0-9a-fA-F]*\\)\\|\\(?:[0-9]+\\(\\.[0-9]+\\)?\\)\\|\\(?:0[0-7]*\\)\\|\\(?:[01]+b\\)\\)\\_>\\)"
        0  zjl-hl-number-face keep)
      '("\\(?:\\.\\|->\\)\\(\\_<\\(?:\\w\\|\\s_\\)+\\_>\\)"
...
(defun my-go-get-local-variables()
  "注:由于cedet 1.1尚未支持golang，本来想自己写一份但难度比较大，因此
   zjl-hl尚未支持对golang中局部变量/参数进行解析高亮， add by mosp 2015/01/02 21:49:32 "
...
(defun my-go-get-local-arguments (&optional point)
      ...
      (let* ((local-variable-list '())
        ...
        (condition-case nil
          (when func-body-begin
            ...
            (when (equal major-mode 'go-mode) ;;if major-mode changed,this hook may still running???
              (setq items (my-go-get-local-variables))
              (unless items
                (semantic-parse-region start end);;force reparse, idea from ecb-method
                (setq items (my-go-get-local-variables)))
              (when items
                (dolist (item items)
                  (setq local-variable-list (cons (car item) local-variable-list))))
              (setq items (my-go-get-local-arguments))
              (unless items
                (semantic-parse-region start end);;force reparse, idea from ecb-method
                (setq items (my-go-get-local-arguments)))
              (when items
            ...
            (save-excursion
              (when parameter-list ;; should before local-variale-list since some parameter will override by local-variable
                (case major-mode
                  ...
                  ('go-mode (zjl-hl-symbol-region func-body-begin end (concat "\\_<" (regexp-opt parameter-list) "\\_>") zjl-hl-parameters-reference-face t t t))                 
              (when local-variable-list
                (case major-mode
                  ...
                  ('go-mode (zjl-hl-symbol-region func-body-begin end (concat "\\_<" (regexp-opt local-variable-list) "\\_>")  zjl-hl-local-variable-reference-face t t t))              
              ))
      ...
      (let (start end)
        ...
        (setq zjl-hl-find-hl-var-arg-regions
              (case major-mode
                ...
                ('c++-mode 'zjl-hl-find-hl-var-arg-regions-c)
                ('go-mode 'zjl-hl-find-hl-var-arg-regions-go)
        ...
        (setq zjl-hl-next-comment-pos
              (case major-mode
                ...
                ('c++-mode 'zjl-hl-next-comment-pos-c)
                ('go-mode 'zjl-hl-next-comment-pos-go)
        ...
        (setq zjl-hl-get-body-begin
              (case major-mode
                ...
                ('c++-mode 'zjl-hl-get-body-begin-c)
                ('go-mode 'zjl-hl-get-body-begin-go)                
...
(defun zjl-hl-enable-global-all-modes ()
  ...
  (when zjl-hl-c++-mode-enable-flag
    (zjl-hl-enable-global 'c++-mode))
  (when zjl-hl-go-mode-enable-flag
    (zjl-hl-enable-global 'go-mode))
...
(defun zjl-hl-get-body-begin-go (start end);;if has no begin, return nil, (zjl-hl-find-regexp-goto-end) may return nil
  (save-excursion
    (goto-char start)
    (zjl-hl-find-regexp-goto-end start end "{")))
...
(defun zjl-hl-find-hl-var-arg-regions-go (start end)
  (save-excursion
    (goto-char start)
    (let (defun-start defun-end regions)
      (while (and (setq defun-end 
...
(defun zjl-hl-next-comment-pos-go (start end)
  (save-excursion
    (let (next-comment-start next-comment-exist comment-pos-1 comment-pos-2 min-pos pos-pair)
      (goto-char start)
      (save-excursion
